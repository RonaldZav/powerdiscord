...
function loadConfig() {
    try {
        ...
        return require(path.join(process.cwd(), 'config.powerdiscord.js'));
    } catch (err) {
        console.error('The config.powerdiscord.js file could not be loaded. Make sure you have the file in the root of the project.');
        process.exit(1);
    }
}

class PowerDiscord extends Client {
    constructor() {
        super({
